...
def calculate_demographic_data(print_data=True):
    ...
    rich_percentage = round(len(num_min_workers[num_min_workers['salary'] == ">50K"]) / len(num_min_workers) * 100, 2)
    

    # What country has the highest percentage of people that earn >50K?
    countries = df[df['salary'] == '>50K']['native-country'].value_counts()
    total_countries = df['native-country'].value_counts()
    highest_earning_country_percentage = (countries / total_countries * 100).max()
